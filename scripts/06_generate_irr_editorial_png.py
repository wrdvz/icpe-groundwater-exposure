...
OUT_PNG = PROJECT_ROOT / "docs" / "parallaxe_icpe_irr_groundwater_exposure_2023.png"
...
LIBELLES = {
    "forte_pression_nappe_baisse": "Forte pression IRR + nappe en baisse",
    "faible_pression_nappe_baisse": "Faible pression IRR + nappe en baisse",
    "forte_pression_nappe_non_baissiere": "Forte pression IRR + nappe non baissière",
    "faible_pression_nappe_non_baissiere": "Faible pression IRR + nappe non baissière",
...
def reclasser_grille_irr(france: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, float]:
    grid = gpd.read_file(EXPOSURE_GRID_GEOJSON_FILE).to_crs(WORK_CRS)
    grid = gpd.clip(grid, france)

    for col in ["station_count", "groundwater_median_variation_20y_cm", "irr_withdrawal_volume_m3"]:
        grid[col] = pd.to_numeric(grid[col], errors="coerce")

    volumes_positifs = grid.loc[grid["irr_withdrawal_volume_m3"] > 0, "irr_withdrawal_volume_m3"]
    ...
    def classer_ligne(row: pd.Series) -> str:
        ...
        forte_pression = pd.notna(row["irr_withdrawal_volume_m3"]) and row["irr_withdrawal_volume_m3"] > seuil_p75
    ...
    grid["classe_irr_png"] = grid.apply(classer_ligne, axis=1)
    return grid, seuil_p75


def charger_points_icpe_irr(france: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, int]:
    ...
    irr = matches[matches["usage_code"] == "IRR"][["code_aiot"]].drop_duplicates()
    points = context.merge(irr, on="code_aiot", how="inner")
...
def ajouter_bloc_texte(
    ...
    fig.text(
        0.07,
        0.93,
        "Exposition des sites ICPE aux tensions d'irrigation sur les eaux souterraines",
        fontsize=28,
        ha="left",
        va="top",
    ...
    fig.text(
        0.07,
        0.895,
        "Grille 20 km croisant les volumes de prélèvements d'irrigation 2023 et la tendance des nappes sur 20 ans (2005-2025)",
        fontsize=18,
        ha="left",
        va="top",
        color=COULEURS["texte"],
    ...
    notes = (
        "Points = sites ICPE dont le meilleur rapprochement BNPE économique relève de l'irrigation (IRR)\n"
        "Forte pression = volume IRR par maille > P75 des mailles non nulles ; baisse = variation médiane des nappes ≤ -10 cm\n"
        f"France métropolitaine ; {formatter_nombre(n_sites)} sites, ramenés à {formatter_nombre(n_points_agreges)} positions ; seuil P75 IRR = {formatter_nombre(seuil_p75)} m3"
    )
    fig.text(
        0.07,
        0.82,
...
def ajouter_legende(fig: plt.Figure, police: FontProperties | None) -> None:
    handles = [
        ...
        Line2D([0], [0], marker="o", color="none", markerfacecolor=COULEURS["points_icpe"], markeredgecolor=COULEURS["points_icpe"], markersize=7, label="Sites ICPE IRR"),
    ]
    leg = fig.legend(
        handles=handles,
        loc="lower right",
...
def dessiner_carte(ax: plt.Axes, france: gpd.GeoDataFrame, grille: gpd.GeoDataFrame, points: gpd.GeoDataFrame) -> None:
    ...
    for classe in ordre:
        sous = grille[grille["classe_irr_png"] == classe]
        if len(sous):
...
def main() -> None:
    ...
    grille, seuil_p75 = reclasser_grille_irr(france)
    points, n_sites = charger_points_icpe_irr(france)
    ...
    print(f"Seuil P75 IRR (m3): {seuil_p75:,.0f}")
    print(f"Sites ICPE IRR affichés: {n_sites:,}")
