...
OUT_PNG = PROJECT_ROOT / "docs" / "parallaxe_icpe_ind_groundwater_exposure_2023.png"
...
LIBELLES = {
    "forte_pression_nappe_baisse": "Forte pression IND + nappe en baisse",
    "faible_pression_nappe_baisse": "Faible pression IND + nappe en baisse",
    "forte_pression_nappe_non_baissiere": "Forte pression IND + nappe non baissière",
    "faible_pression_nappe_non_baissiere": "Faible pression IND + nappe non baissière",
...
def silhouette_france_metropolitaine() -> gpd.GeoDataFrame:
    ...
    metro = regions[regions["code"].isin(METRO_REGION_CODES)].copy()
    metro = metro.to_crs(WORK_CRS)
    return gpd.GeoDataFrame({"nom": ["France métropolitaine"], "geometry": [metro.union_all()]}, crs=WORK_CRS)


def reclasser_grille_ind(france: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, float]:
    grid = gpd.read_file(EXPOSURE_GRID_GEOJSON_FILE).to_crs(WORK_CRS)
    grid = gpd.clip(grid, france)

    for col in ["station_count", "groundwater_median_variation_20y_cm", "ind_withdrawal_volume_m3"]:
        grid[col] = pd.to_numeric(grid[col], errors="coerce")

    volumes_positifs = grid.loc[grid["ind_withdrawal_volume_m3"] > 0, "ind_withdrawal_volume_m3"]
    ...
    def classer_ligne(row: pd.Series) -> str:
        ...
        forte_pression = pd.notna(row["ind_withdrawal_volume_m3"]) and row["ind_withdrawal_volume_m3"] > seuil_p75
    ...
    grid["classe_ind_png"] = grid.apply(classer_ligne, axis=1)
    return grid, seuil_p75


def charger_points_icpe_ind(france: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, int]:
    matches = pd.read_csv(ICPE_BNPE_ECONOMIC_BEST_MATCH_FILE, low_memory=False)
    context = pd.read_csv(ICPE_GROUNDWATER_CONTEXT_FILE, low_memory=False)

    matches["code_aiot"] = matches["code_aiot"].astype(str)
    context["code_aiot"] = context["code_aiot"].astype(str)

    ind = matches[matches["usage_code"] == "IND"][["code_aiot"]].copy()
    ind = ind.drop_duplicates(subset=["code_aiot"])

    points = context.merge(ind, on="code_aiot", how="inner")
    points = points[points["site_sector"] == "Industrie"].copy()
    ...
    points_agg = (
        ...
        .agg(
            ...
            n_signaux_solides=("is_signal_solid", lambda s: int(pd.Series(s).fillna(False).sum())),
        )
    )
...
def ajouter_bloc_texte(
    fig: plt.Figure,
    police: FontProperties | None,
    n_sites: int,
    n_points_agreges: int,
    seuil_p75: float,
) -> None:
    ...
    fig.text(
        0.07,
        0.93,
        "Exposition des sites ICPE industriels aux tensions sur les eaux souterraines",
        fontsize=28,
        ha="left",
        va="top",
    ...
    fig.text(
        0.07,
        0.895,
        "Grille 20 km croisant les volumes de prélèvements industriels 2023 et la tendance des nappes sur 20 ans (2005-2025)",
        fontsize=18,
        ha="left",
        va="top",
        color=COULEURS["texte"],
    ...
    notes = (
        "Points = sites ICPE du secteur Industrie dont le meilleur rapprochement BNPE économique est industriel (IND)\n"
        "Forte pression = volume IND par maille > P75 des mailles non nulles ; baisse = variation médiane des nappes ≤ -10 cm\n"
        f"France métropolitaine ; {formatter_nombre(n_sites)} sites, ramenés à {formatter_nombre(n_points_agreges)} positions ; seuil P75 IND = {formatter_nombre(seuil_p75)} m3"
    )
    fig.text(
        0.07,
        0.82,
...
def ajouter_signature(fig: plt.Figure, police: FontProperties | None) -> None:
    ...
    fig.text(
        0.07,
        0.07,
        f"Carte établie le {aujourd_hui}\nParallaxe processing",
        fontsize=12.5,
        ha="left",
        va="bottom",
        color=COULEURS["texte"],
...
def ajouter_legende(fig: plt.Figure, police: FontProperties | None) -> None:
    patch_kwargs = {"fontproperties": police} if police else {}
    handles = [
        ...
        Line2D([0], [0], marker="o", color="none", markerfacecolor=COULEURS["points_icpe"], markeredgecolor=COULEURS["points_icpe"], markersize=7, label="Sites ICPE IND"),
    ]
    leg = fig.legend(
        handles=handles,
        loc="lower right",
        bbox_to_anchor=(0.93, 0.08),
        frameon=False,
        fontsize=12,
        handlelength=2.0,
        labelspacing=0.65,
...
def dessiner_carte(ax: plt.Axes, france: gpd.GeoDataFrame, grille: gpd.GeoDataFrame, points: gpd.GeoDataFrame) -> None:
    ...
    for classe in ordre:
        sous = grille[grille["classe_ind_png"] == classe]
        if len(sous):
    ...
    if len(points):
        ax.scatter(
            points.geometry.x,
            points.geometry.y,
            s=points["taille_point"],
            c=COULEURS["points_icpe"],
            alpha=0.6,
            linewidths=0,
            zorder=3,
        )
...
def main() -> None:
    ...
    grille, seuil_p75 = reclasser_grille_ind(france)
    points, n_sites = charger_points_icpe_ind(france)
    ...
    print(f"Seuil P75 IND (m3): {seuil_p75:,.0f}")
    print(f"Sites ICPE Industrie affichés: {n_sites:,}")
