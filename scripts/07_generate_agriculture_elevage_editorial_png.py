...
OUT_PNG = PROJECT_ROOT / "docs" / "parallaxe_icpe_agriculture_elevage_groundwater_exposure_2023.png"
...
def charger_points_agri_elevage(france: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, int]:
    points = pd.read_csv(ICPE_ENRICHED_FILE, low_memory=False)
    points = points[points["categorie_eau_7"] == "Agriculture et élevage"].copy()
...
def ajouter_bloc_texte(fig: plt.Figure, police: FontProperties | None, n_sites: int, n_points_agreges: int, seuil_p75: float) -> None:
    ...
    fig.text(
        0.07,
        0.93,
        "Exposition des sites agricoles et d'élevage aux tensions sur les eaux souterraines",
        fontsize=28,
        ha="left",
        va="top",
    ...
    notes = (
        "Points = sites ICPE classés en Agriculture et élevage via la NAF\n"
...
def ajouter_legende(fig: plt.Figure, police: FontProperties | None) -> None:
    handles = [
        ...
        Line2D([0], [0], marker="o", color="none", markerfacecolor=COULEURS["points_icpe"], markeredgecolor=COULEURS["points_icpe"], markersize=7, label="Sites Agriculture et élevage"),
    ]
...
def main() -> None:
    ...
    points, n_sites = charger_points_agri_elevage(france)
    ...
    print(f"Sites Agriculture et élevage affichés: {n_sites:,}")
